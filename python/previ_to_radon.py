...
def ReadCommandLine(argv):
    ...
    databasegroup.add_option(
        "--port", action="store", type="string", default="5432", help="Database port"
    )
    ...
    if options.analysis_time != None:
        try:
            options.analysis_time = datetime.datetime.strptime(
                options.analysis_time, "%Y%m%d%H%M%S"
            )
...
WHERE
    ...
    if row == None or len(row) == 0:
        error = "Table not found for producer_id = %s, analysis_time = %s" % (
            producer_id,
            analysis_time,
        )
...
def Insert(infile, tableInfo, stations):
    ...
    for line in reader:
        ...
        try:
            ...
            print(
                "No database station id found for local station id %s at network %d"
                % (line[2], options.network_id)
            )
            continue
        if not options.dry_run:
            ...
            try:
                insertcur.execute(
                    "EXECUTE insertplan (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)", line
                )
                # 				if options.show_sql:
                ...
                if e.pgcode == "23505":
                    ...
                    updatecur.execute(
                        "EXECUTE updateplan (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)", line
                    )
                    updates += 1
        ...
        if totalrows % commit_chunk == 0:
            ...
            print(
                "%s cumulative inserts: %d, updates: %d, lines per second: %d"
                % (
                    datetime.datetime.now(),
                    inserts,
                    updates,
                    commit_chunk / GetTotalSeconds(stop_time - start_time),
                )
            )
    ...
    if not options.dry_run:
        cur.execute(
            "ANALYZE " + tableInfo["schema_name"] + "." + tableInfo["partition_name"]
        )
