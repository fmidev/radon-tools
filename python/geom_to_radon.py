...
def update_table_meta_grid(grb, geom_id, name):
    ...
    if rows is None or len(rows) == 0:
        print(
            (
                "No earlier rows for producer %s found from table table_meta_grid, returning"
                % (prod_id)
            )
        )
        return
    ...
    for row in rows:
        print(
            "Found existing definition from table_meta_grid for this producer but different geometry:"
        )
        print(
            (
        ...
        proceed = input(
            "Insert line for geometry %s using these same attributes? " % (name)
        )
        ...
        cur.execute(
            "INSERT INTO table_meta_grid(producer_id, schema_name, table_name, geometry_id, retention_period, partitioning_period, analysis_times) VALUES (%s, %s, %s, %s, %s, %s, %s)",
            (row[0], row[1], row[2], geom_id, row[3], row[4], row[5]),
        )
...
def rll(grb):
    ...
    print(
        (
            "First point: %s,%s\nNi: %d\nNj: %d\nDi: %s\nDj: %s\nScanning mode: %s\nSouth pole: %s,%s"
            % (lon, lat, ni, nj, di, dj, scmode, splon, splat)
        )
    )
    ...
    # print cur.mogrify(query, (geom_id, name, ni, nj, lon, lat, di, dj, scmode, desc))
    cur.execute(
        query, (geom_id, name, ni, nj, lon, lat, di, dj, scmode, desc, splon, splat)
    )
...
def ps(grb):
    ...
    print(
        (
            "First point: %s,%s\nNi: %d\nNj: %d\nDi: %s\nDj: %s\nScanning mode: %s\nOrientation: %s"
            % (lon, lat, ni, nj, di, dj, scmode, orient)
        )
    )
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        ...
        print(
            "interactive tool to insert geometry information from grib to radon database"
        )
        sys.exit()
