...
bool GribCreate::Create(const string &theDate, const string &hour)
{
	...
	{
		query << "CREATE TABLE " << table_suffix[i] << " (dset_id number(10) not null,"
		...
		query << "CREATE public synonym " << table_suffix[i] << " for " << table_suffix[i] << " GRANT ALL on "
		      << table_suffix[i] << " to adm_bdap "
		...
		query << "INSERT into grid_table"
		      ...
		      << " VALUES (" << type_id[i] << ", " << table_suffix[i] << ")";
		cout << query.str() << "\n";
		query.str("");
		...
		query << "INSERT into as_grid"
		      ...
		      << " VALUES"
		      << " (:dset_id, " << table_suffix[i] << "," << mgroup["model_type"] << ", " << geom_name[i] << ",'AF',"
		      << " to_date(" << theDate << "||to_char(" << mgroup["reseau"] << ",'09')||'0000')," << retention[i]
		      << ",0,sysdate,"
...
void GribCreate::DryRun(bool theDryRun) { itsDryRun = theDryRun; }
bool GribCreate::DryRun() { return itsDryRun; }
